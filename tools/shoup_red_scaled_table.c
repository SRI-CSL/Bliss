/*
 * Tables in Shoup-style format for combined NTT/product
 * by powers of psi, using Longa and Naehrig's reduction.
 *
 ...
 *
 * Output
 * - w[t + j] = psi^(n/2t) * phi^(n/2t)^j * inverse(k)
 *   for t=1, 2, 4, ..., n/2
 *   and j=0, ... t-1
 */
...
/*
 * Build the table for n, q, phi, psi, inv_k
 */
static void build_scaled_shoup_red_table(uint32_t *a, uint32_t n, uint32_t q, uint32_t phi, uint32_t psi, uint32_t inv_k) {
  uint32_t t, j, i;
  uint32_t x, y;
  ...
  i = 1;
  for (t=1; t<n; t <<= 1) {
    // we don't assume psi^2 = phi here 
    x = power(psi, n/(2*t), q);
    x = (x * inv_k) % q;
    y = power(phi, n/(2*t), q);
    for (j=0; j<t; j++) {
...
int main(int argc, char *argv[]) {
  ...
  build_scaled_shoup_red_table(table, n, q, phi, psi, inv_k);
  print_table(stdout, "shoup_red_scaled_ntt", table, n, q, false);
  print_table(stdout, "shoup_sred_scaled_ntt", table, n, q, true);
