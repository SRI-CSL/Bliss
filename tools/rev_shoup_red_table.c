 ...
 *
 * Write q as  (k*2^m + 1) where k is not a multiple of 2.
 *
 * Output: table of powers of phi scaled by inverse(k)
 * - w[t + j] = phi^(n/2t)^bitreverse(j) * inverse(k)
 *    for t=1, 2, 4, ..., n/2
 *    and j=0, ... t-1
 */
 ...
 */
static void build_rev_shoup_red_table(uint32_t *a, uint32_t n, uint32_t q, uint32_t phi, uint32_t inv_k) {
  uint32_t t, j, i, k;
  uint32_t x, y;
...
int main(int argc, char *argv[]) {
  ...
  build_rev_shoup_red_table(table, n, q, phi, inv_k);
  print_table(stdout, "rev_shoup_red_ntt", table, n, q, false);
  print_table(stdout, "rev_shoup_sred_ntt", table, n, q, true);
