 ...
 * - w[t + j] = psi^(n/2t) * phi^(n/2t)^bitrev(j) * inverse(k)
 *    for t=1, 2, 4, ..., n/2
 *    and j=0, ... t-1
 */
...
/*
 * Build the table for n, q, phi, psi, and inv_k
 */
static void build_rev_scaled_shoup_red_table(uint32_t *a, uint32_t n, uint32_t q, uint32_t phi, uint32_t psi, uint32_t inv_k) {
  uint32_t t, j, i, k;
  uint32_t x, y;
...
int main(int argc, char *argv[]) {
  ...
  build_rev_scaled_shoup_red_table(table, n, q, phi, psi, inv_k);
  print_table(stdout, "rev_shoup_red_scaled_ntt", table, n, q, false);
  print_table(stdout, "rev_shoup_sred_scaled_ntt", table, n, q, true);
