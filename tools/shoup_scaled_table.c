...
/*
 * Build the table for n, q, phi, and psi
 */
static void build_scaled_shoup_table(uint32_t *a, uint32_t n, uint32_t q, uint32_t phi, uint32_t psi) {
  uint32_t t, j, i;
  uint32_t x, y;
...
int main(int argc, char *argv[]) {
  ...
  build_scaled_shoup_table(table, n, q, phi, psi);
  print_table(stdout, "shoup_scaled_ntt", table, n, q);
